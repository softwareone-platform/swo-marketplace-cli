...
class SettingsData(BaseDataModel):
    ...
    settings_items: list[SettingsItem] = field(default_factory=list)
    json_path: str | None = None

    @classmethod
    @override
    def from_dict(cls, row_data: dict[str, Any]) -> Self:
        return cls(settings_items=[SettingsItem.from_dict(row_data)])
    ...
    def from_json(cls, json_data: dict[str, Any]) -> Self:
        ...
        settings_items = [
            SettingsItem.from_json({"name": key, "value": formatted_settings.get(setting_path)})
            for key, setting_path in constants.SETTINGS_API_MAPPING.items()
        ]
        return cls(settings_items=settings_items)

    @override
    def to_json(self) -> dict[str, Any]:
        settings: dict[str, Any] = {}
        for setting_item in self.settings_items:
