...
class ProductService(BaseService):
    ...
    def create(self) -> ServiceResult:
        ...
        try:
            new_product_data = self.api.post(data=multipart_payload, headers=headers)
    ...
    def export(self, resource_id: str) -> ServiceResult:
        ...
        settings_excel_file_manager.add(product.settings.settings_items)
    ...
    def update(self) -> ServiceResult:
        ...
        setting_items = [
            ...
            for settings_item in setting_data.settings_items  # type: ignore[attr-defined]
            if settings_item.action == DataActionEnum.UPDATE
        ]

        try:
            self.api.update(product.id, SettingsData(settings_items=setting_items).to_json())
        except MPTAPIError as e:
            self._set_error(str(e))
            return ServiceResult(success=False, errors=[str(e)], model=None, stats=self.stats)
