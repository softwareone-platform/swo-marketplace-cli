...
def mock_mpt_client(mocker):
    return mocker.patch("cli.plugins.audit_plugin.app.CLIMPTClient", autospec=True)
...
def test_diff_by_object_id_successful_comparison(mock_mpt_client, mocker, sample_audit_records):
    ...
    mock_get_records.assert_called_once_with(mock_mpt_client.return_value, "obj123", 10)
    assert "Comparing audit trails..." in result.stdout
    assert "old_value" in result.stdout
    assert "new_value" in result.stdout


def test_diff_by_object_id_custom_positions(mock_mpt_client, mocker, sample_audit_records):
    mock_get_records = mocker.patch(
        "cli.plugins.audit_plugin.app.get_audit_records_by_object", autospec=True
    )
    mock_get_records.return_value = sample_audit_records
...
def test_diff_by_object_id_insufficient_records(mock_mpt_client, mocker, sample_audit_records):
    mock_get_records = mocker.patch(
        "cli.plugins.audit_plugin.app.get_audit_records_by_object", autospec=True
    )
...
def test_diff_by_object_id_invalid_positions(mock_mpt_client, mocker, sample_audit_records):
    mock_get_records = mocker.patch(
        "cli.plugins.audit_plugin.app.get_audit_records_by_object", autospec=True
    )
    mock_get_records.return_value = sample_audit_records
...
def test_diff_by_records_id_successful_comparison(mock_mpt_client, mocker, sample_audit_records):
    ...
    assert result.exit_code == 0
    mock_get_trail.assert_any_call(mock_mpt_client.return_value, "audit1")
    mock_get_trail.assert_any_call(mock_mpt_client.return_value, "audit2")
    assert "Comparing audit trails..." in result.stdout
    assert "old_value" in result.stdout
    assert "new_value" in result.stdout


def test_diff_by_records_id_different_objects(mock_mpt_client, mocker):
    mock_get_trail = mocker.patch("cli.plugins.audit_plugin.app.get_audit_trail", autospec=True)
