...
def write_accounts_file(account_file_path, accounts):
    ...
    account_file_path.write_text(json.dumps(accounts))
    return account_file_path


def write_accounts(account_file_path, accounts):
    return write_accounts_file(
        account_file_path,
        [account.model_dump() for account in accounts],
    )


def test_selects_active_account(account_file_path, active_vendor_account, inactive_vendor_account):
    file_path = write_accounts(account_file_path, [inactive_vendor_account, active_vendor_account])
...
def test_selects_account_by_id(account_file_path, active_vendor_account, inactive_vendor_account):
    file_path = write_accounts(account_file_path, [active_vendor_account, inactive_vendor_account])
...
def test_unknown_account_id_raises_error(account_file_path, active_vendor_account):
    file_path = write_accounts(account_file_path, [active_vendor_account])
...
def test_no_active_account_raises_error(account_file_path, inactive_vendor_account):
    file_path = write_accounts(account_file_path, [inactive_vendor_account])
...
def test_missing_required_field_raises_error(
    account_file_path, active_vendor_account, missing_field
):
    raw_account = active_vendor_account.model_dump()
    raw_account.pop(missing_field)
    write_accounts_file(account_file_path, [raw_account])
...
def test_unknown_extra_fields_are_tolerated(account_file_path, active_vendor_account):
    raw_account = {**active_vendor_account.model_dump(), "extra": "field"}
    write_accounts_file(account_file_path, [raw_account])

    result = load_account(account_file_path)

    assert result == active_vendor_account


def test_default_path_reads_home_swocli(mocker, account_file_path, active_vendor_account):
    write_accounts_file(account_file_path, [active_vendor_account.model_dump()])
