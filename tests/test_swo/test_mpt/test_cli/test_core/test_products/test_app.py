...
def test_create_product_error(mocker, product_container_mock):
    stats = product_container_mock.config.stats
    create_product_mock = mocker.patch.object(
        product_container_mock.product_service(),
        "create",
...
def test_export_product(tmp_path, product_container_mock):
...
def test_export_product_account_not_allowed(mocker, product_container_mock):
    mocker.patch.object(
        product_container_mock, "account", return_value=Mock(is_operations=Mock(return_value=False))
    )
...
def test_export_product_error_exporting_product(mocker, product_container_mock, operations_account):
    product_container_mock.stats = mocker.patch.object(ProductStatsCollector, "has_errors", True)
