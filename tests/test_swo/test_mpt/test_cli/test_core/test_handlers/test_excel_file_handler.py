...
def excel_file_handler(tmp_path, workbook):
    ...
    with patch(
        "swo.mpt.cli.core.handlers.excel_file_handler.load_workbook", return_value=workbook
    ):
...
@pytest.mark.parametrize(("file_path", "expected_path"), [
    ("non_extension_file", "non_extension_file.xlsx"),
    ("test_file.xlsx", "test_file.xlsx"),
])
...
def test_get_values_for_dynamic_sheet(excel_file_handler):
    ...
    assert len(data[0]) == 1
    assert data[0] == [("A2", "Header1", "Value1")]
