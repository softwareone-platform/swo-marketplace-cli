...
def export(
    ...
    active_account = container.account()
    ...
    for product_id in product_ids:
        ...
        with console.status(f"Exporting product with id: {product_id}..."):
            ...
            container.item_service().export()
            container.item_group_service().export()
            container.parameter_group_service().export()
            container.agreement_parameters_service().export()
            container.item_parameters_service().export()
            container.request_parameters_service().export()
            container.subscription_parameters_service().export()
            container.template_service().export()
...
def sync_product(
    ...
    if product is None or force_create:
        ...
        with console.status("Update product...") as status:
            container.config["resource_id"].from_value(product.id)
...
def update_product(container, status):
    status.update(f"Update item for {container.config.resource_id}...")
