...
class ExcelFileHandler(FileHandler):
    ...
    def get_data_from_horizontal_sheet(
        ...
        for row in sheet_iter:
            ...
            yield [
                (row[index].coordinate, column_name, row[index].value)
                for index, column_name in enumerate(header_fields)
                if fields is None or column_name in fields
            ]

            # TODO: Re-think the return format. Should we use a dictionary instead?
            # yield {
            #     cell.coordinate: {header_fields[index]: cell.value}
